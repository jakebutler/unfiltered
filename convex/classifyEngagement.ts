...
export const classifyEngagement = action({
  ...
  handler: async (ctx, args) => {
    ...
    const minimaxModel = (process.env.MINIMAX_MODEL ?? "MiniMax-Text-01").trim().replace(/^['"]+|['"]+$/g, "") || "MiniMax-Text-01";
    if (!minimaxApiKey) {
    ...
    try {
      const response = await client.chat.completions.create({
        // MiniMax model is configurable; default keeps current known-good behavior.
        model: minimaxModel,
        messages: [
